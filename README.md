...
- 应用允许用户创建多个本地账号；每个账号拥有一个 Agent 配置、多个项目和多个 bug 线程，账号之间隔离项目、线程、设置、凭据、隐私授权与记忆。
- Plan、权限、Provider/模型和数据策略属于账号下的 Agent，跨该账号的项目与线程共享；项目只提供工作区、命令、验证和路径事实。
- 同一账号一次只允许一个线程运行。运行期间禁止切换账号、卸载工作区、删除相关数据或正常关闭应用，必须由用户手动停止。
